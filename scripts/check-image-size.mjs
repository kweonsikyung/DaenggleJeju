...
files.forEach((file) => {
  ...
  if (fileSizeInBytes > MAX_SIZE_BYTES) {
    ...
    console.error(`\x1b[31m[Image Size Error]\x1b[0m`);
    console.error(`📄 파일명: ${file}`);
    console.error(`⚖️ 현재 용량: ${fileSizeInKB}KB (제한: ${MAX_SIZE_KB}KB)`);
    console.error(`---------------------------------------------------------`);
    console.error(`💡 해상도는 유지하면서 용량만 줄이려면 다음 방법을 권장합니다:`);
    console.error(`1. https://tinypng.com 사이트에서 스마트 압축 진행`);
    console.error(`2. 확장자를 .webp 또는 .avif로 변환하여 저장`);
    console.error(`---------------------------------------------------------`);
