...
`apps/web`과 `packages/daenggle-ui`는 각자 tsconfig를 갖고 있었지만 `target`, `strict`, `moduleResolution` 같은 기반 옵션이 사실상 동일했다. 패키지가 늘어날수록 이 중복이 누적되고, 한쪽에서 옵션을 바꿔도 다른 쪽에 전파되지 않아 패키지 경계에서 타입 오류가 발생한다. 설정 변경의 단일 진원지가 없으면 유지보수 시 동기화 실수가 구조적으로 생길 수밖에 없다.

### hooks

`useModal`, `useWebShare`, `useTypingEffect`는 특정 페이지나 비즈니스 로직과 무관한 범용 유틸리티다. `apps/web/src/hooks/`에 있으면 다른 패키지가 참조할 수 없고, 앱 전체 릴리즈에 종속돼 독립적인 버전 관리나 테스트가 불가능하다. 앱 코드와 공유 코드가 같은 경계 안에 섞이면 패키지 분리 구조의 의미가 없어진다.
